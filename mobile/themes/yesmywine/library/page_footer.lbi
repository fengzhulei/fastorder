...
#scrollUp {
	...
	color: #eee;
	font-size: 40px;
	line-height: 1;text-align: center;text-decoration: none;bottom: 1.4em;right: 10px;overflow: hidden;width: 46px;
	height: 46px;
	border: none;
	opacity: 0.6;
}
...
<script language="javascript">
	...
	function openSearch(){
		if($(".con").is(":visible")){
			if($(".top_menu_m50").size() > 0)
			{
				$(".top_menu_m50").css("marginTop","50px");
			}
			if($(".top_menu_m60").size() > 0)
			{
				$(".top_menu_m60").css("marginTop","60px");
			}	
			$(".con").hide();	
			$(".search").show();
		}
	...
	function closeSearch(){
		if($(".con").is(":hidden")){
			if($(".top_menu_m50").size() > 0)
			{
				$(".top_menu_m50").css("marginTop","0px");
			}
			if($(".top_menu_m60").size() > 0)
			{
				$(".top_menu_m60").css("marginTop","0px");
			}
			$(".con").show();	
			$(".search").hide();
		}
